...
import net.dromard.common.util.OrderedHashtable;
 ...
 * @version 0.1, 07/06/2005
 */
public class PropertiesWithComments extends OrderedHashtable {
     ...
     * @serial
     */
    protected Hashtable<String, String> comments = new Hashtable<String, String>();
    protected int commentsKey = 0;
     ...
     */
    public PropertiesWithComments(Properties defaults) {
    ...
    public String getComment(String key) {
        ...
        if(comment != null) {
            if(comment.charAt(1) == ' ') return comment.substring(2);
    ...
    public void setComment(String key, String comment) {
        if(comment.charAt(0) == '#') comments.put(key, comment);
        else comments.put(key, "# "+comment);
    }
    
    public void addComment(String key, String comment) {
        String c = getComment(key);
        if(c != null) setComment(key, c+"\n# "+comment);
        else setComment(key, comment);
    }
        ...
        while((line = in.readLine()) != null) {
            if (line.length() > 0) {
                ...
                if ((firstChar != '#') && (firstChar != '!')) {
                    ...
                    // Add comment
                    if(comment != null) {
                        comments.put(key, comment.toString());
                        comment = null;
                    }
                } else {
                    if(line.length() > 1 && line.charAt(1) != '!') {
                        ...
                        // Blank lines are ignored
                        if (keyStart != len) {
                            if(comment == null) comment = new StringBuffer(line.substring(keyStart));
                            else comment.append("\n"+line.substring(keyStart));
                        }
                    }
                }
            } else {
                if(comment != null) put("#"+(++commentsKey), comment.toString());
                comment = null;
            }
        }
    }
    ...
    public synchronized void store(OutputStream out, String header)
        ...
        for (Enumeration e = keys(); e.hasMoreElements();) {
            ...
            if(key.charAt(0) == '#') {
                ...
                // Manage comment for key:
                String comment = comments.get(key);
                if(comment != null) writeln(awriter, comment);
                // Print key/value
    ...
    private synchronized void enumerate(Hashtable<String, String> h) {
        if (defaults != null) {
            for (Enumeration e = defaults.keys(); e.hasMoreElements();) {
                String key = (String) e.nextElement();
                h.put(key, (String) defaults.get(key));
            }
        }
        for (Enumeration e = keys(); e.hasMoreElements();) {
            String key = (String) e.nextElement();
    ...
    public static void main(String[] args) {
        try {
            ...
            PropertiesWithComments p = new PropertiesWithComments();
            System.out.println(new File(fileName).getAbsolutePath());
            p.load(new FileInputStream(new File(fileName)));
            p.list(System.out);
            p.store(new FileOutputStream(new File("test."+fileName)), "Storing of 'test.properties' files");
            //little.test
        } catch (Exception e) {
            e.printStackTrace();
        }
